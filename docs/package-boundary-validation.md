...
| Validation depends on no hosted vendor | `CompositionValidator` and every `Rule` take only a composition, a brand, an optional asset store, and an optional capability. No clock, no network, no model. Twelve failing fixtures run offline. | Passed |
